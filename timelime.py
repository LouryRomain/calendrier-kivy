...
class TimeTick(Tick):  
    ...
    def pos_of(self, tickline, time):
        print(self.pos_index_of(tickline, time)[0])
...
class TimelineApp(App):
	def __init__(self,**kwargs):
		...
		now = local_now().astimezone(UTC)
		self.timeline.center_on_timeframe(now ,now+ timedelta(days=1))
		self.timeline.labeller_cls.time_halign='right'
		self.popup=Popup(content=self.box)
		self.popup.open()
